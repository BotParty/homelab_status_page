...
forked runtime -> exec workers on server by default using bun / deno

why? fs = 1,000 faster than network requests -> browsers = 16GB max - server = 1terabyte ram

automate os + real_sockets/real_fs/real_hardware (gpus = 1 million times faster in next 20 years + robotics = 100 trillion dollar market = both async)
