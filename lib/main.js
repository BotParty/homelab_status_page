...
import {texture, updateTexture, makeTexture} from './Texture';
...
async function init(options={}) {
  ...
  function compute() {
    if (! state.commandEncoder) state.commandEncoder = device.createCommandEncoder();
    

    if (! state.computePass) state.computePass = commandEncoder.beginComputePass()

    const blockDim = 
    computePass.setPipeline(state.computePipeline);
    computePass.setBindGroup(0, state.computeConstants);

    computePass.setBindGroup(1, state.computeBindGroup0);
    computePass.dispatchWorkgroups(
      Math.ceil(srcWidth / blockDim),
      Math.ceil(srcHeight / batch[1])
    );

    computePass.setBindGroup(1, state.computeBindGroup1);
    computePass.dispatchWorkgroups(
    ...
    for (let i = 0; i < 10; ++i) {
      computePass.setBindGroup(1, state.computeBindGroup2);
      computePass.dispatchWorkgroups(
        Math.ceil(srcWidth / blockDim),
        Math.ceil(srcHeight / batch[1])
      );
