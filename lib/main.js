...
async function init(options={}) {
  ...
  function draw(newState) {
    ...
    if (Array.isArray(newState)) 
      return newState.map((ctx) => draw(ctx))
  ...
  // return draw;
  //return context with initdrawcall and buffer and prop
  return {
    initDrawCall, 
    buffer, 
    prop
...
function buffer(args) {
  if (! (this instanceof buffer)) return new buffer(args);
  this.args = args
}
function prop(name) {
  return name
...
function scopes() {}
export default { init }; //insight: each draw call has its own scope and render pass descriptor
