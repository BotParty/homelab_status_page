...
var<private> rand_stuff : vec2<f32>;


fn rand() -> f32 {
  rand_stuff.x = fract(cos(dot(rand_stuff, vec2<f32>(24.14077926, 22.61690225))) * 164.8168);
  rand_stuff.y = fract(cos(dot(rand_stuff, vec2<f32>(54.47856553, 35.84153136))) * 542.7645);
  return rand_stuff.y;
}
...
fn simulate(
  ...
  var rand_stuff = (sim_params.seed.xy + vec2<f32>(GlobalInvocationID.xy)) * sim_params.seed.zw;
  ...
  // respawned.
  particle.color = vec4<f32>(1., .5, 0, 1);
